-- Load data from HDFS
inputDialogues4 = LOAD 'hdfs:///user/ranjeet/inputs/episodeIV_dialogues.txt' USING PigStorage('\t') AS (name:chararray, line:chararray);
inputDialogues5 = LOAD 'hdfs:///user/ranjeet/inputs/episodeV_dialogues.txt' USING PigStorage('\t') AS (name:chararray, line:chararray);
inputDialogues6 = LOAD 'hdfs:///user/ranjeet/inputs/episodeVI_dialogues.txt' USING PigStorage('\t') AS (name:chararray, line:chararray);
...
rmf hdfs:///user/ranjeet/outputs;

-- Store result in HDFS
STORE namesOrdered INTO 'hdfs:///user/ranjeet/outputs' USING PigStorage('\t');
